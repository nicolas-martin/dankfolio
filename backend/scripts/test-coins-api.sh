...
echo -e "${CYAN}curl -s \"${BASE_URL}/api/coins\"${NC}"
curl -s "${BASE_URL}/api/coins" | python3 -m json.tool || echo -e "${RED}Error: Failed to fetch coins${NC}"

# Test 2: Get coin by ID (SOL)
echo -e "\n${YELLOW}Test 2: Get SOL coin by ID${NC}"
echo -e "${CYAN}curl -s \"${BASE_URL}/api/coins/So11111111111111111111111111111111111111112\"${NC}"
curl -s "${BASE_URL}/api/coins/So11111111111111111111111111111111111111112" | python3 -m json.tool || echo -e "${RED}Error: Failed to fetch SOL coin${NC}"

# Test 3: Get coin by ID (USDC)
echo -e "\n${YELLOW}Test 3: Get USDC coin by ID${NC}"
echo -e "${CYAN}curl -s \"${BASE_URL}/api/coins/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v\"${NC}"
curl -s "${BASE_URL}/api/coins/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v" | python3 -m json.tool || echo -e "${RED}Error: Failed to fetch USDC coin${NC}"

# Test 4: Get non-existent coin (should return 404)
echo -e "\n${YELLOW}Test 4: Get non-existent coin (should return 404)${NC}"
echo -e "${CYAN}curl -s \"${BASE_URL}/api/coins/xyz123\"${NC}"
echo "Response content:"
curl -s "${BASE_URL}/api/coins/xyz123"
echo ""
response=$(curl -s -o /dev/null -w "%{http_code}" "${BASE_URL}/api/coins/xyz123")
...
echo -e "${CYAN}curl -s \"${BASE_URL}/api/coins/JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN\"${NC}"
curl -s "${BASE_URL}/api/coins/JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN" | python3 -m json.tool || echo -e "${RED}Error: Failed to fetch JUP token data${NC}"
