...
import (
	...
	// HTTP/2 and h2c are not needed when using ListenAndServeTLS
)
...
// Start starts the Connect RPC server with TLS
func (s *Server) Start(port int, certFile, keyFile string) error {
	...
	// Start HTTPS server with CORS middleware
	addr := fmt.Sprintf(":%d", port)
	log.Printf("Starting Connect RPC HTTPS server on %s using cert %s and key %s", addr, certFile, keyFile)

	// Wrap the mux with CORS middleware
	finalHandler := middleware.CORSMiddleware(s.mux)

	// Use ListenAndServeTLS for HTTPS
	return http.ListenAndServeTLS(addr, certFile, keyFile, finalHandler)
}
