...
func PostRequest[T any](c *Client, ctx context.Context, requestURL string, requestBody any) (T, error) {
	...
	if err != nil {
		return zeroT, fmt.Errorf("failed to marshal request body: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, "POST", requestURL, bytes.NewReader(bodyBytes)) // Method is hardcoded to POST
	if err != nil {
		return zeroT, fmt.Errorf("failed to create POST request: %w", err)
	}
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return zeroT, fmt.Errorf("failed to perform POST request to %s: %w", requestURL, err)
	}
	defer resp.Body.Close()
