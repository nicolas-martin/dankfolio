...
func NewService(ss *solana.SolanaTradeService) *Service {
	return &Service{
		solanaService: ss,
...
func (s *Service) GetTradeQuote(ctx context.Context, fromCoinID, toCoinID string, amount float64) (*TradeQuote, error) {
	// Validate input
	if fromCoinID == "" || toCoinID == "" || amount <= 0 {
		return nil, fmt.Errorf("invalid trade parameters")
	}

	// Get quote from Solana service
	estimatedAmount, exchangeRate, err := s.solanaService.GetSwapQuote(ctx, fromCoinID, toCoinID, amount)
	if err != nil {
