...
const Notification = memo(({ type, message, onDismiss }) => {
...
const TradeScreen = ({ route, navigation }) => {
        ...
        const fetchTradeQuote = async (amount, fromId, toId, trigger = 'Manual input') => {
                ...
                try {
                        // Get the from coin to determine decimals
                        const fromCoinObj = getCoinById(fromId);
                        if (!fromCoinObj) {
                                throw new Error('From coin not found');
                        }

                        // Convert amount based on token decimals
                        const decimals = fromCoinObj.decimals || 9; // Default to 9 for SOL
                        const amountInDecimals = Math.floor(numAmount * Math.pow(10, decimals));
                        ...
                        const response = await api.getTradeQuote(fromId, toId, amountInDecimals);

                        // Get the to coin to determine decimals for conversion
                        const toCoinObj = getCoinById(toId);
                        if (!toCoinObj) {
                                throw new Error('To coin not found');
                        }

                        // Convert estimated amount back from decimals
                        const toDecimals = toCoinObj.decimals || 9; // Default to 9 for SOL
        ...
        const initializeCoins = useCallback(() => {
                ...
                // Find coins by symbol and get their mint addresses
                ...
                // Use coin IDs as mint addresses
                const newFromCoin = fromCoinObj?.id || '';
                const newToCoin = toCoinObj?.id || '';

                console.log('Initializing coins:', {
                        fromSymbol: initialFromCoin,
                        toSymbol: initialToCoin,
                        fromMint: newFromCoin,
                        toMint: newToCoin
                });
        ...
        const handleTradeSubmit = async () => {
                ...
                console.log('Starting trade with:', {
                        ...
                        toCoin,
                        amount: fromAmount,
                        amountInLamports: Math.floor(parseFloat(fromAmount) * LAMPORTS_PER_SOL)
                });
                ...
                try {
                        ...
                        // Call buildAndSignSwapTransaction with correct parameter order
                        const signedTxBase64 = await buildAndSignSwapTransaction(
                                fromCoin,  // inputMint
                                toCoin,    // outputMint
                        ...
                        if (!signedTxBase64) throw new Error('Failed to create and sign transaction');

                        const fromCoinSymbol = getCoinById(fromCoin)?.symbol || 'Unknown';
                        const toCoinSymbol = getCoinById(toCoin)?.symbol || 'Unknown';

                        // Execute the trade with the base64 encoded transaction
                        const result = await api.executeTrade(fromCoin, toCoin, parsedAmount, signedTxBase64);
        ...
        const renderCoinItem = (coinId, isFrom) => {
                const coinObj = getCoinById(coinId);
                if (!coinObj) return null;

                const iconUri = getCoinIcon(coinObj);
                const amountComponent = isFrom ? (
                        <AmountInput
                                value={fromAmount}
...
const styles = StyleSheet.create({
        ...
        balanceText: { color: '#9F9FD5', fontSize: 12, marginTop: 4 },
        ...
        tradeButtonText: { color: '#FFFFFF', fontSize: 16, fontWeight: 'bold' },
        notification: {
                position: 'absolute',
                top: 20,
                left: 20,
        ...
        notificationText: { color: '#FFFFFF', fontSize: 14, flex: 1 },
