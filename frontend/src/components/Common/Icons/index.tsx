...
// Smart trend icon that automatically picks the right icon and color
interface TrendIconProps extends IconBaseProps {
...
export const getTrendColor = (
	...
	if (value === undefined) return neutralColor;
...
export const TrendIcon: React.FC<TrendIconProps> = ({ 
	...
	if (value > 0) {
		return <TrendUpIcon color={positiveColor} {...props} />;
	} else if (value < 0) {
		return <TrendDownIcon color={negativeColor} {...props} />;
	}
	
	return null; // No icon for zero change
}; 
