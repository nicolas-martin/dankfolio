...
export async function createAndSignSwapTransaction(
  ...
  try {
    ...
    // Narrow the union type based on the existence of partialSign.
    if ('partialSign' in transaction) {
      transaction.partialSign(wallet);
    } else {
      // When signing, ensure the wallet is treated as a Signer.
      const signers = [wallet] as import('@solana/web3.js').Signer[];
