...
export function formatSummary(issueCount: number, cleanCount?: number): string {
	...
	return chalk.green.bold(`⚠️  Found ${issueCount} ${issueCount === 1 ? 'issue' : 'issues'} across ${cleanCount} clean files:\n\n`);
}
...
export function formatFinalSummary(items: SummaryItem[], totalClean: number): string {
	...
	// for item that is clean, change the label color
	const formattedLabels: string[] = [];
	let totalIssues: number = 0;
	items.forEach(item => {
		if (item.label.includes('clean')) {
			formattedLabels.push(chalk.green.bold(`   • ${item.count} ${item.label}`));
		}
		if (item.label.includes('with issues')) {
			formattedLabels.push(chalk.yellow.bold(`   • ${item.count} ${item.label}`));
			totalIssues += item.count;
		}
	})

	// Calculate total items (clean + items with issues)
	const total = totalClean + totalIssues;

	if (totalIssues === 0) {
		return chalk.green.bold(`\n✨ All ${totalClean} items are clean!\n`);
	}
	formattedLabels.push(chalk.yellow.bold(`   • ${totalIssues} total diagnostic issues`));
	formattedLabels.push(chalk.white.bold(`   • ${total} total items`));

	return `\n📊 Summary:\n${formattedLabels.map(label => label).join('\n')} \n`;
}
