...
async function handleListTrades(options?: FetchInit) {
	...
	const mockTrades = [
		create(TradeSchema, {
			...
			type: 'swap',
			amount: 2.5,
			outputAmount: 246.125, // 2.5 SOL * 98.45 = 246.125 USDC
			price: 98.45,
			fee: 0.005,
			status: 'completed',
		...
		create(TradeSchema, {
			...
			type: 'swap',
			amount: 100.0,
			outputAmount: 11235813.0, // 100 USDC / 0.0000089 BONK price = ~11.2M BONK
			price: 1.0,
			fee: 0.25,
			status: 'completed',
		...
		create(TradeSchema, {
			...
			type: 'swap',
			amount: 50.0,
			outputAmount: 0.4425, // 50 JUP * 0.87 / 98.45 SOL price = ~0.44 SOL
			price: 0.87,
			fee: 0.1,
			status: 'pending',
		...
		create(TradeSchema, {
			...
			type: 'swap',
			amount: 1000.0,
			outputAmount: 0.0, // Failed trade, no output
			price: 0.000042,
			fee: 0.002,
			status: 'failed',
		...
		create(TradeSchema, {
			...
			type: 'swap',
			amount: 500.0,
			outputAmount: 0.615, // 500 MOON * 0.00123 = 0.615 USDC
			price: 0.00123,
			fee: 0.001,
			status: 'completed',
